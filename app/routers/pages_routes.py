...
# Copied from your Capstone version (kept here so pages look the same)
STYLE = """
<style>
...
/* Quantity display pill (used on single-drink pages) */
...
def _find_drink(drink_id: str):
    """Return drink dict by id, or None."""
    ensure_drinks_file()
...
def dashboard(request: Request):
    username = request.session.get('user') or request.session.get('username') or 'Guest'
...
<div class='card'>
  ...
  <div class='btnrow'>
    <a class='secondary' href='{CANVA_URL}' target='_blank' rel='noopener'>Open Canva</a>
  </div>
</div>
...
def builder(request: Request):
    ...
    template = """<html><head><title>Builder</title>{STYLE}</head>
    <body><div class='page'>
  ...
  drinks.forEach(d => {
    ...
    card.innerHTML = `
      ...
      <div class='btnrow' style='margin-top:12px'>
        ...
        <button class='secondary' onclick="addToCart('${d.id}','${d.name}',${d.calories})">Add</button>
...
async function checkout(){
  ...
  if(!data.ok){ status.innerText = 'Error: ' + (data.error || 'Unknown'); return; }
...
(async function init(){
  ...
  renderCart();
    ...
    html = template.replace("{STYLE}", STYLE).replace("{{USER}}", user)
    return HTMLResponse(html)


@router.get("/drink/{drink_id}", response_class=HTMLResponse)
def drink_page(request: Request, drink_id: str):
    """Single-drink ordering page.

    This is made for Canva deep-links: each drink has its own URL.
    """
    user = _require_user(request)
    if not user:
        # keep it simple: require login (your Canva link can still point here;
        # after login the user can go back).
        return RedirectResponse("/login", status_code=302)

    d = _find_drink(drink_id)
    if not d:
        return HTMLResponse(f"<html><head><title>Not Found</title>{STYLE}</head><body><div class='page'><h1>NOT FOUND</h1><div class='card'><p class='small'>Unknown drink id: <b>{drink_id}</b></p><div class='btnrow'><button class='secondary' onclick=\"window.location.href='/builder'\">Back</button></div></div></div></body></html>", status_code=404)

    name = d.get("name", drink_id)
    cal = int(d.get("calories", 0) or 0)

    # NOTE: We intentionally avoid Python f-strings for HTML/JS templates here,
    # because JavaScript uses many `{ ... }` braces which break f-string parsing.
    tpl = Template("""
...
def history(request: Request):
    ...
    if not orders:
        ...
        rows = "<table class='table'><tr><th>Drink</th><th>Qty</th><th>Calories</th><th>Time</th></tr>" + "".join([
            f"<tr><td>{o.get('drinkName','')}</td><td>{o.get('quantity',1)}</td><td>{o.get('calories',0)}</td><td>{o.get('ts','')}</td></tr>"
...
def drink_links_page(request: Request):
    """Show all per-drink links for easy copy/paste into Canva."""
    user = _require_user(request)
    if not user:
        return RedirectResponse("/login", status_code=302)
    ...
    drinks = load_drinks()
    rows = "".join([
        f"<tr><td>{d.get('name','')}</td><td><span class='pill'>{d.get('calories',0)} cal</span></td><td><a href='/drink/{d.get('id')}'><b>/drink/{d.get('id')}</b></a></td></tr>"
        for d in drinks
    ])
    ...
    <body><div class='page'>
      <h1>DRINK LINKS</h1>
            <div class='card'>
        <h2>ALL DRINK PAGES</h2>
        <div class='small'>Each drink has its own URL. Use the <b>path</b> on your deployed domain.</div>
        <table class='table'>
...
def recommendations_page(request: Request):
    ...
    # Old behaviour: top ordered drinks
    ...
    # New: ML-ish recommendations (new drinks you haven't tried)
