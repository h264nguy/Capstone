...
<body><div class='page'>
  ...
  <div class='card'>
    ...
    <div class='small'>Pick a mood to personalize recommendations.</div>
...
function setMainEtaFromOrders(orders){
  ...
  const bar = document.getElementById('etaFill');
  ...
  const etas = list.map(o => (o.etaSeconds==null ? 0 : __adjSec(o.etaSeconds)));
  const aheads = list.map(o => (o.etaAheadSeconds==null ? 0 : __adjSec(o.etaAheadSeconds)));

  const totalRemain = Math.max(...etas);
  const startsIn = list.some(o => o.status === 'in_progress') ? 0 : Math.max(0, Math.min(...aheads));

  // stable denominator per snapshot
  if(!window.__topDenom || window.__topDenomTs !== __mySnapshotTs){
    window.__topDenom = Math.max(1, totalRemain);
    window.__topDenomTs = __mySnapshotTs;
  }
  const denom = window.__topDenom;
  const pct = Math.max(0, Math.min(100, Math.round(((denom - totalRemain)/denom)*100)));
  bar.style.width = pct + '%';

  const suffix = (startsIn > 0) ? ` (starts in ~${formatETA(startsIn)})` : ' (starts now)';
  txt.textContent = `${formatETA(totalRemain)}${suffix}`;
}
...
function __renderMyQueue(box){
  const orders = __myOrdersSnapshot || [];
  if(!box) return;
  if(!orders.length){
  ...
  box.innerHTML = orders.map(o => {
    ...
    const ahead = aheadRaw===null ? null : __adjSec(aheadRaw);
    const eta   = etaRaw===null   ? null : __adjSec(etaRaw);

    const startsTxt = ahead===null ? '' : ` (starts in ${formatETA(ahead)})`;
    const rightTxt = (eta===null) ? '' : `${formatETA(eta)}${startsTxt}`;

    let bar = '';
    if((o.status === 'in_progress') && etaRaw!==null && eta!==null){
    ...
    return `
      <div class="orderRow">
        <div>
          ...
          <div class="small">Order ID: ${escapeHtml(String(o.orderId || o.id || ''))}</div>
        </div>
...
function ingredientsHtml(d, enableHighlight=true){
  ...
  const items = arr.map(x => {
    ...
    return `<li style="${style}">${escapeHtml(key)}</li>`;
  }).join("");
  return `<div class='ing'><div class='small'>Ingredients:</div><ul>${items}</ul></div>`;
}

function whyHtml(d){
  const arr = (d && Array.isArray(d.why)) ? d.why : [];
  const items = (arr && arr.length>0)
    ? arr.map(x => `<li>${String(x)}</li>`).join("")
    : `<li>Popular choice</li>`;
  return `
    <details class='why'>
      <summary class='small'>Why recommended?</summary>
      <ul>${items}</ul>
    </details>
  `;
}
...
// UI feedback
  ...
  try {
    ...
    let res = await fetch('/checkout', {
      ...
      body: JSON.stringify({items, mood: (window.currentMood || null)})
    });
    if(res && res.status===404){ res = await fetch('/api/checkout', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({items, mood: (window.currentMood || null)}) }); }
    ...
    if(data && data.ok){
      ...
      if(status) status.innerText = 'Order placed!' + (orderId ? ` Order ID: ${orderId}` : '');
      ...
      if(orderId){ try{ startQueuePoll(orderId); }catch(e){} try{ startEtaTicker(); }catch(e){} }
...
(async function init(){
  ...
  window.currentMood = window.currentMood || null;
  let moodTimer = null;
  ...
  function setMoodButtons(){
    document.querySelectorAll('.mood-btn').forEach(btn=>{
      ...
      btn.classList.toggle('active', !!window.currentMood && m === window.currentMood);
    });
  }

  async function loadRecs(){
    try{
      const url = window.currentMood
        ? `/api/recommendations?k=5&mood=${encodeURIComponent(window.currentMood)}`
        : '/api/recommendations?k=5';
      ...
      if(rdata && rdata.ok){
        ...
        if(banner){
          ...
          if(based){
            ...
            const dbg = (rdata.based_on_ingredients||[]).map(x=>String(x)).join(', ');
    ...
    <body><div class='page'>
      ...
      <script>
        ...
        async function pickMood(mood){{
          ...
          status.textContent = mood ? ("Mood: " + mood) : "Mood cleared (default recommendations).";

          const url = mood ? ("/api/recommendations?mood=" + encodeURIComponent(mood) + "&k=5") : "/api/recommendations?k=5";
          try{{
