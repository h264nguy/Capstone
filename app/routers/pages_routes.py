...
def dashboard(request: Request):
    ...
    # NOTE: Removed the extra big "MENU" dashboard card (your screenshot),
    # but /builder (menu) still exists in the code.
...
# Alias: if you ever click /menu, it goes to /builder
...
def drink_page(request: Request, drink_id: str):
    ...
    if not d:
        return HTMLResponse(
            f"<html><head><title>Not Found</title>{STYLE}</head><body><div class='page'><h1>NOT FOUND</h1><div class='card'><p class='small'>Unknown drink id: <b>{drink_id}</b></p><div class='btnrow'><button class='secondary' onclick=\"window.location.href='/builder'\">Back</button></div></div></div></body></html>",
            status_code=404,
        )
