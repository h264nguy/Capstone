...
def esp_complete(body: CompleteBody, key: str):
    ...
    We require that the current unit has been 'in_progress' for at least ETA_SECONDS_PER_DRINK seconds.
    """
    _check_key(key)
    ...
    target = None
    for o in q:
        if str(o.get("id")) == str(body.id) and o.get("status") in ("pending", "in_progress"):
            target = o
            break
