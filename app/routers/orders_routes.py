...
async def checkout(request: Request):
    ...
    for it in items:
        ...
        # store in history
        orders.append({k: line[k] for k in ("username","drinkId","drinkName","quantity","calories","ts")})
        # also enqueue for ESP polling (so orders can be made even when site is published)
        enqueue_esp_order(line)
    save_orders(orders)
