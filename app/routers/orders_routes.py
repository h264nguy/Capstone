...
from app.core.storage import load_orders, save_orders, enqueue_esp_order, queue_position
...
async def checkout(request: Request) -> JSONResponse:
    ...
    return JSONResponse(
        {"ok": True, "saved": True, "count": len(norm_items), "queued": True, "orderId": order_id, "queue": pos},
        status_code=200,
    )
